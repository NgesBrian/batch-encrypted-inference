...
int main(int argc, char *argv[]) {
    ...
    auto conv3_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[0], dataWidths[1],  kernelSize, paddingLen, stridingLen);
    auto conv4_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[0], dataWidths[1],  kernelSize, paddingLen, shortcutStridingLen);
    auto conv5_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[0], dataWidths[2],  kernelSize, paddingLen);
    auto avg_keys = annBatchController.generate_avgpool_batch_optimized_rotation_positions(batchSizes[0], dataWidths[2], avgpoolSize, avgpoolSize, true, rotPositions);
    auto convert_keys = annBatchController.generate_secure_convert_batch_input_rotation_positions(batchSizes[0],  channels[3], dataWidths[3]);
    auto fc_keys = annBatchController.generate_fullyconnected_batch_rotation_positions(batchSizes[0], {channels[4]}, {channels[3]}, fcRotIndex);
    /************************************************************************************************ */
    ...
    fc_layer.push_back(avg_keys);
    fc_layer.push_back(convert_keys);
    ...
    for (int idx = 0; idx < 1; idx++) {
        ...
        cout <<"Block 2 " << endl;
        convData = resnet_block(fheController, annBatchController, "layer2_block2", convData, batchSizes[1], dataWidths[1], dataSizes[1], channels[2], channels[2], reluScale, true, false);
        cout <<"Block 3 " << endl;
        ...
        fheController.clear_context(slotsSizes[1]);
        fheController.load_bootstrapping_and_rotation_keys(slotsSizes[2], "layer3.bin", false);


        cout <<"Block 2 " << endl;
        convData = resnet_block(fheController, annBatchController, "layer3_block2", convData, batchSizes[2], dataWidths[2], dataSizes[2], channels[3], channels[3], reluScale, true, false);
        cout <<"Block 3" << endl;
        ...
        startIn = get_current_time();
        convData = annBatchController.secure_batch_globalAvgPool(convData, batchSizes[2], dataWidths[2], channels[3], avgpoolSize, rotPositions);
        cout<< "Ciphertext Conversion" << endl;
        auto fcData = annBatchController.secure_convert_batch_input(convData, batchSizes[2], channels[3], dataWidths[3]);
        measuringTime.push_back(measureTime(startIn, get_current_time()));
        cout<< "Classification" << endl;
        ...
        fcData = fc_layer_block(fheController, annBatchController, "layer_fc", fcData, batchSizes[2], channels[3], channels[4], fcRotIndex);
        ...
        auto predictions = fheController.read_batch_inferencedLabel(fcData, batchSizes[2], channels[4], outFile);
