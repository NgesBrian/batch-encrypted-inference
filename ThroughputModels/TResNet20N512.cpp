...
int main(int argc, char *argv[]) {
    ...
    vector<int> batchSizes = {32, 128, 512};
    int pipeVal = 4;
    int rotPositions = 32;
    int fcRotIndex = 100;
    ...
    auto conv5_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[2], dataWidths[2],  kernelSize, paddingLen);
    auto avg_keys = annBatchController.generate_avgpool_batch_optimized_rotation_positions(batchSizes[2], dataWidths[2], avgpoolSize, avgpoolSize, true, rotPositions);
    auto convert_keys = annBatchController.generate_secure_convert_batch_input_rotation_positions(batchSizes[2],  channels[3], dataWidths[3]);
    auto fc_keys = annBatchController.generate_fullyconnected_batch_rotation_positions(batchSizes[2], {channels[4]}, {channels[3]}, fcRotIndex);
    ...
    fheController.generate_bootstrapping_and_rotation_keys(serconverter_layer, slotsSizes[2], "convert_layer.bin", true);
    fheController.clear_context(slotsSizes[2]);
    ...
    for (int idx = 0; idx < 1; idx++) {
        ...
        auto multi_fcData = fc_layer_block(fheController, annBatchController, "layer_fc", fcData, batchSizes[2], channels[3], channels[4], fcRotIndex);
        ...
        auto predictions = fheController.read_batch_inferencedLabel_multiple_outputs(multi_fcData, batchSizes[2], channels[4], outFile);
