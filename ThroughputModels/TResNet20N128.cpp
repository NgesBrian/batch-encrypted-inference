...
int inputbatchSize = 32;
vector<int> slotsSizes = {15, 15, 15, 15};
int scalingFact = 2; 
...
int main(int argc, char *argv[]) {
    ...
    /********************************* generate rotation keys positions ************************** */
    ...
    auto conv5_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[1], dataWidths[2],  kernelSize, paddingLen);
    auto avg_keys = annBatchController.generate_avgpool_batch_optimized_rotation_positions(batchSizes[1], dataWidths[2], avgpoolSize, avgpoolSize, true, rotPositions);
    auto convert_keys = annBatchController.generate_secure_convert_batch_input_rotation_positions(batchSizes[1],  channels[3], dataWidths[3]);
    auto fc_keys = annBatchController.generate_fullyconnected_batch_rotation_positions(batchSizes[1], {channels[4]}, {channels[3]}, fcRotIndex);
    ...
    vector<vector<int>> rkeys_layer1, rkeys_layer2, rkeys_layer3, convert_layer, fc_layer;
    ...
    cout << "imagesData Size: " << imagesData.size() << endl;
    ofstream outFile;
    ...
    for (int idx = 0; idx < 1; idx++) {
        ...
        for(int tIdx=0; tIdx < pipeVal; tIdx++){
            ...
            for (int b = 0; b < batchSizes[0]; b++) {
                ...
                cout << "Loading Image Index: " << fImgIndx << endl; 
                auto img = imagesData[fImgIndx];
                // cout << "loaded from Buffer: " << fImgIndx << endl; 
                batchedImages.push_back(img);
                // cout << "Pushed to Batch: " << tIdx << endl; 
            }
        ...
        printDuration(inference_time, "run time", false);
        printTimeWithMessage("ResNet20 Circuit Blocks: ", measuringTime);

        startIn = get_current_time();
        convData = fheController.batch_bootstrap_function(convData, channels[3], bootstrap_level);
        intermTime.push_back(measureTime(startIn, get_current_time()));
        
        cout << "Global Pooling" << endl;
        fheController.clear_context(slotsSizes[2]);
        fheController.load_bootstrapping_and_rotation_keys(slotsSizes[3], "layer3.bin", false);

        startIn = get_current_time();
        convData = annBatchController.secure_batch_globalAvgPool(convData, batchSizes[1], dataWidths[2], channels[3], avgpoolSize, rotPositions);
        measuringTime.push_back(measureTime(startIn, get_current_time()));
        printTimeWithMessage("ResNet20 Circuit + GlobalPool: ", measuringTime);

        cout << "Ciphertext Converter" << endl;
        fheController.clear_context(slotsSizes[2]);
        fheController.load_bootstrapping_and_rotation_keys(slotsSizes[3], "convert_layer.bin", false);
        auto fcData = annBatchController.secure_convert_batch_input(convData, batchSizes[1], channels[3], dataWidths[3]);
        printTimeWithMessage("ResNet20 Converting Layer: ", measuringTime);
        ...
        fcData = fc_layer_block(fheController, annBatchController, "layer_fc", fcData, batchSizes[1], channels[3], channels[4], fcRotIndex);
        ...
        auto predictions = fheController.read_batch_inferencedLabel(fcData, batchSizes[1], channels[4], outFile, imgIdx);
...
Ctext fc_layer_block(FHEController &fheController, ANNBatchController &annBatchController, string layer, Ctext encrytedInput, int batchSize, int inputChannels, int outputChannels, int rotPositions){
    ...
    for(int i=0; i < outputChannels; i++){
        auto encodeWeights = fheController.encode_inputData(fc_rawKernel[i], encrytedInput->GetLevel());
        fc_kernelData.push_back(encodeWeights);
    }
    Ptext fc_BiasData = fheController.encode_inputData(fc_rawBiasData, encrytedInput->GetLevel());
