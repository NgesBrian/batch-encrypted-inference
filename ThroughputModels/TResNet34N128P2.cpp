...
int main(int argc, char *argv[]) {
    ...
    auto conv7_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[1], dataWidths[3],  kernelSize, paddingLen, stridingLen);
    auto conv8_keys = annBatchController.generate_convolution_batch_rotation_positions(batchSizes[1], dataWidths[3],  kernelSize, paddingLen, shortcutStridingLen);

    auto avg_keys = annBatchController.generate_avgpool_batch_optimized_rotation_positions(batchSizes[1], dataWidths[3], avgpoolSize, avgpoolSize, true, rotPositions);
    auto convert_keys = annBatchController.generate_secure_convert_batch_input_rotation_positions(batchSizes[1],  channels[4], dataWidths[4]);
    auto fc_keys = annBatchController.generate_fullyconnected_batch_rotation_positions(batchSizes[1], {channels[5]}, {channels[4]}, fcRotIndex);
    /************************************************************************************************ */
...
/********************************************************************************************************************************************/;
    ...
    for (int idx = 0; idx < 1; idx++) {
        ...
        cout <<"Block 2 " << endl;
        convData = resnet_block(fheController, annBatchController, "layer2_block2", interConvData, batchSizes[1], dataWidths[1], dataSizes[1], channels[2], channels[2], reluScale, true, false);
        ...
        cout <<"Block 2 " << endl;
        convData = resnet_block(fheController, annBatchController, "layer3_block2", convData, batchSizes[1], dataWidths[2], dataSizes[2], channels[3], channels[3], reluScale, true, false);
        cout <<"Block 3" << endl;
        convData = resnet_block(fheController, annBatchController, "layer3_block3", convData, batchSizes[1], dataWidths[2], dataSizes[2], channels[3], channels[3], reluScale, true, false);
        cout <<"Block 4" << endl;
        convData = resnet_block(fheController, annBatchController, "layer3_block4", convData, batchSizes[1], dataWidths[2], dataSizes[2], channels[3], channels[3], reluScale, true, false);
        cout <<"Block 5" << endl;
        convData = resnet_block(fheController, annBatchController, "layer3_block5", convData, batchSizes[1], dataWidths[2], dataSizes[2], channels[3], channels[3], reluScale, true, false);
        cout <<"Block 6" << endl;
        convData = resnet_block(fheController, annBatchController, "layer3_block6", convData, batchSizes[1], dataWidths[2], dataSizes[2], channels[3], channels[3], reluScale, true, false);
        // fheController.read_batch_minmaxValue(convData, channels[3], totalSize);
        printDuration(inference_time, "run time", false);
        
        cout<< endl<<  "Layer 4" << endl;
        cout <<"Block 1 " << endl;
        convData = resnet_block(fheController, annBatchController, "layer4_block1", convData, batchSizes[1], dataWidths[2], dataSizes[2], channels[3], channels[4], reluScale, true, true);
        totalSize = batchSizes[1] * dataSizes[3];

        cout <<"Block 2 " << endl;
        convData = resnet_block(fheController, annBatchController, "layer4_block2", convData, batchSizes[1], dataWidths[3], dataSizes[3], channels[4], channels[4], reluScale, true, false);
        cout <<"Block 3" << endl;
        convData = resnet_block(fheController, annBatchController, "layer4_block3", convData, batchSizes[1], dataWidths[3], dataSizes[3], channels[4], channels[4], reluScale, true, false);
        ...
        fheController.clear_context(slotsSizes[2]);
        fheController.load_bootstrapping_and_rotation_keys(slotsSizes[4], "layer4.bin", false);
        startIn = get_current_time();


        cout << " Global Pooling" << endl;
        convData = annBatchController.secure_batch_globalAvgPool(convData, batchSizes[1], dataWidths[3], channels[4], avgpoolSize, rotPositions);
        measuringTime.push_back(measureTime(startIn, get_current_time()));

        cout << "Ciphertext Converter..." << endl;
        fheController.clear_context(slotsSizes[2]);
        fheController.load_bootstrapping_and_rotation_keys(slotsSizes[4], "convert_layer.bin", false);
        auto fcData = annBatchController.secure_convert_batch_input(convData, batchSizes[1], channels[4], dataWidths[4]);
        ...
        fheController.clear_context(slotsSizes[2]);
        fheController.load_bootstrapping_and_rotation_keys(slotsSizes[4], "fc_layer.bin", false);
        ...
        auto predictions = fheController.read_batch_inferencedLabel_multiple_outputs(multi_fcData, batchSizes[1], channels[5], outFile);
